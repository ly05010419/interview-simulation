...
st.markdown(
    ...
    "with built-in security guards to prevent misuse."
)
...
if st.session_state.interview_started:
    ...
    if user_input:
        ...
        if len(user_input) > MAX_INPUT_LENGTH:
            st.warning("⚠️ Answer is too long. Please keep it concise.")
            st.stop()

        # ---------- Input Validation Guard ----------
        if not validate_user_input(user_input):
            st.warning(
                "⚠️ Input rejected. Please provide a relevant interview answer."
            )
            st.stop()
